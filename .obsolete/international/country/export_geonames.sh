...
--output international/countrydatas/population_continent_preview.txt
...
--output international/countrydatas/population_country_preview.txt
...
python sql_to_fwf.py --db world-datas-analysis.db \
--query "$QUERY" --comment "Source: $SOURCE" \
--output international/countrydatas/population_city_preview.txt
...
python sql_to_fwf.py --db world-datas-analysis.db \
--query "$QUERY" --comment "Source: $SOURCE" \
--output international/countrydatas/population_continent_sample_all_columns.txt
...
--block-by "continentname" \
--comment "Grouped by continent" \
--output international/countrydatas/population_country_sample_all_columns.txt

# Export city
QUERY="select * from v_geonames_city ORDER BY population DESC LIMIT 1000;"
python sql_to_fwf.py --db world-datas-analysis.db \
--query "$QUERY" --comment "Source: $SOURCE" \
--output international/countrydatas/population_city_sample_all_columns.txt