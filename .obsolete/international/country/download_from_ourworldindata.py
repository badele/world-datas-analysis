...
def download_indicator_description(conn):
    extractor = Extractor.from_yaml_file(
        "./international/country/ourworldindata_category.yaml"
    )
    r = requests.get("https://ourworldindata.org/charts")
    ...
    for block in data["block"]:
        ...
        categories.append((catid, "ourworldindata", "EN", category))

        for idx in range(len(block["indicator"])):
            indicator = block["indicator"][idx]
            id = block["link"][idx].replace('https://ourworldindata.org/grapher/','')
            indicators.append(("EN", catid, id, indicator))

    df_cat = pd.DataFrame(categories, columns=["CATID", "source", "lang", "category"])
    df_cat.to_sql("db_category", conn, if_exists="replace", index=False)
    ...
    print(df_indicator.to_string())

    df_indicator.to_sql("db_indicator", conn, if_exists="replace", index=False)
