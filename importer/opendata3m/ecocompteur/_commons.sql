...
CREATE OR REPLACE VIEW wda_opendata3m_ecocompteur_observations AS
    SELECT eo.serie,el.city,el.name, el.lat,el.lon,el.democraty_district,el.district,eo.ts,eo.nb_observations
...
INSERT OR REPLACE INTO wda_scopes
    SELECT 'opendata3m', 'wda_opendata3m_ecocompteur_observations', 'city', city as cityname, COUNT(*)
    FROM wda_opendata3m_ecocompteur_observations
    GROUP BY city;


INSERT INTO wda_datasets
    SELECT 'opendata3m','opendata3m','wda_opendata3m_ecocompteur_observations', 'city', 'ecocompteur observations', 'https://data.montpellier3m.fr/dataset/comptages-velo-et-pieton-issus-des-eco-compteurs/resource/edf3e04f-9409-40fe-be66', (SELECT column_count FROM duckdb_views WHERE view_name='wda_opendata3m_ecocompteur_observations') as nb_vars, COUNT(*), COUNT(DISTINCT city)
    FROM wda_opendata3m_ecocompteur_observations;
